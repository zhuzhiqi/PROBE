...
unsigned int high_level_synthesis(char *infile,char* process,bool FUs,bool valid_signal){
  ...
  if(strstr(infile,".sc") || strstr(infile,".cpp"))
    ...
    sprintf(cmd,"bdltran -EE -lic_wait=120 -Zresource_fcnt=GENERATE -Zresource_mcnt=GENERATE -tcio -Zdup_reset=YES %s -c%d -s -lfl /eda/cwb/cyber_561/LINUX/packages/asic_45.FLIB  -lb /eda/cwb/cyber_561/LINUX/packages/asic_45.BLIB  %s.IFF -oI  >>LOG_HLS",string_v, CLOCK*100,  process);
...
void gen_run_rtl_sim(bool gen,bool run,unsigned int cycles,char* process,char* hdl){
  ...
  if(gen==GEN){
	...
	*/
	...
	{
    sprintf(cmd, "tbgen %s_E.IFF -output=file -b10 -compare=transaction	 \
					-finish	 \
					-scr=modelsim-all  -dump=VCD 	 \
					-dump_signal=PORT:ALL 			 \
					-lic_wait=120 %s >>LOG_Sim", process,hdl);
	//printf("print = %s\n",cmd);
	system(cmd); 
	}
    ...
    */
    ...
    sprintf(cmd, "tbgen %s_E.IFF -max_cycle=100000  -output=file -b10 	\
					...
					-scr=modelsim-all  -dump=VCD 	\
					-dump_signal=PORT:ALL 			\
					-lic_wait=120 -cycles=%d %s >>LOG_Sim", process,cycles,hdl);
	system(cmd); 
